...
from utils.BCP_utils import context_mask, mix_loss, mix_loss_new, mix_loss_sep, parameter_sharing, update_ema_variables, sup_mask_loss, random_mask, concate_mask

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='data/LA', help='Name of Dataset')
parser.add_argument('--exp', type=str, default='All', help='exp_name')
...
def cut_mask(out, thres=0.5):
    probs = F.softmax(out, 1)
    masks = (probs >= thres).type(torch.int64).contiguous()
    return masks
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # loss_ce = F.cross_entropy(outputs, lab_a)
            # loss_dice = DICE(outputs, lab_a)
            # loss = loss_ce
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # mixll_img = img_a * (1 - img_mask) + img_b * img_mask
            # mixuu_img = unimg_a * img_mask + unimg_b * (1 - img_mask)
            mixll_img = img_a * img_mask + img_b * (1 - img_mask)
            mixuu_img = unimg_a * img_mask + unimg_b * (1 - img_mask)
            ...
            outputs_ll, _ = model(mixll_img)
            outputs_uu, _ = model(mixuu_img)
            # outputs_ma = outputs_l * img_mask
            # outputs_mb = outputs_u * (1-img_mask)
            # outputs_fa = 0.8 * outputs_a + 0.2 * outputs_ma
            # outputs_fb = 0.8 * outputs_b + 0.2 * outputs_mb
            with torch.no_grad():
                lab_l = get_cut_mask(outputs_l, nms=0)
                lab_u = get_cut_mask(outputs_u, nms=0)
                lab_ll = get_cut_mask(outputs_ll, nms=0)
                # lab_uu = get_cut_mask(outputs_uu, nms=1)
                a_mask_o = torch.eq(lab_ll, lab_a).float() * img_mask
                a_mask_m = torch.eq(lab_l, lab_a).float() * img_mask
                b_mask_o = torch.eq(lab_ll, lab_b).float() * (1 - img_mask)
                b_mask_m = torch.eq(lab_u, lab_b).float() * (1 - img_mask)
                mask_a = ((a_mask_m + a_mask_o)>1).type(torch.float) * img_mask
                mask_b = ((b_mask_m + b_mask_o)>1).type(torch.float) * (1 - img_mask)

                # mask_ua = (mask_a - lab_l * img_mask)
                # mask_la = (mask_a - lab_ll * img_mask)
                # mask_ub = (mask_b - lab_u * (1 - img_mask))
                # mask_lb = (mask_b - lab_ll * (1 - img_mask))
                # mask_aa = (((a_mask_m + a_mask_o) < 1).type(torch.float)) * img_mask
                # mask_bb = (((b_mask_m + b_mask_o) < 1).type(torch.float)) * (1-img_mask)
                # mask_ua = (mask_a - a_mask_m) * 0.15
                # mask_la = (mask_a - a_mask_o) * 0.15
                # mask_ub = (mask_b - b_mask_m) * 0.15
                # mask_lb = (mask_b - b_mask_o) * 0.15

            # loss_cea = (CE(outputs_l, lab_a) * mask_ua).sum() / (mask_ua.sum() + 1e-16) + (CE(outputs_ll, lab_a) * mask_la).sum() / (mask_la.sum() + 1e-16)
            # loss_ceb = (CE(outputs_u, lab_b) * mask_ub).sum() / (mask_ub.sum() + 1e-16) + (CE(outputs_ll, lab_b) * mask_lb).sum() / (mask_lb.sum() + 1e-16)
            ...
            # loss_ll = mix_loss(outputs_ll, lab_a, lab_b, loss_mask, u_weight=1.0)
            # loss_uu = mix_loss(outputs_uu, plab_a, plab_b, loss_mask, l_weight=args.u_weight, u_weight=args.u_weight)
            # loss_ua = sup_mask_loss(outputs_l, lab_a, mask_ua)
            # loss_la = sup_mask_loss(outputs_ll, lab_a, mask_la)
            # loss_ub = sup_mask_loss(outputs_u, lab_b, mask_ub)
            # loss_lb = sup_mask_loss(outputs_ll, lab_b, mask_lb)

            loss_l = mix_loss_sep(outputs_l, lab_a, plab_a, loss_mask, mask_p=mask_b, u_weight=args.u_weight)
            loss_u = mix_loss_sep(outputs_u, lab_b, plab_b, 1 - loss_mask, mask_p=mask_a, u_weight=args.u_weight)
            loss_ll = mix_loss_sep(outputs_ll, lab_a, lab_b, loss_mask, u_weight=1.0)
            loss_uu = mix_loss_sep(outputs_uu, plab_a, plab_b, mask_a, mask_p=mask_b, l_weight=args.u_weight, u_weight=args.u_weight)
            # loss_l = mix_loss_new(outputs_l, lab_a, plab_a, loss_mask, mask_p=mask_a, u_weight=args.u_weight)
            # loss_u = mix_loss_new(outputs_u, lab_b, plab_b, 1 - loss_mask, mask_p=mask_b, u_weight=args.u_weight)
            # loss_ll = mix_loss_new(outputs_ll, lab_a, lab_b, loss_mask, mask_p=mask_a, mask_n=mask_b, u_weight=1.0)
            # loss_uu = mix_loss_new(outputs_uu, plab_a, plab_b, loss_mask, l_weight=args.u_weight, u_weight=args.u_weight)
            # loss_consist = (DICE(outputs_l, lab_ll, loss_mask) + DICE(outputs_u, lab_ll, 1-loss_mask)) * 0.25
            # loss_ma = mix_loss(outputs_fa, lab_a, lab_a, loss_mask, u_weight=0)
            # loss_mb = mix_loss(outputs_fb, lab_b, lab_b, 1-loss_mask, u_weight=0)

            loss = loss_l + loss_u + loss_ll + loss_uu
            ...
            # writer.add_scalar('Self/loss_ll', loss_ll, iter_num)
            # writer.add_scalar('Self/loss_uu', loss_uu, iter_num)
            writer.add_scalar('Self/loss_all', loss, iter_num)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # logging.info('iteration %d : loss: %03f, loss_l: %03f, loss_u: %03f, loss_ll: %03f, loss_uu: %03f, loss_ua: %03f, loss_la: %03f, loss_ub: %03f, loss_lb: %03f'%(iter_num, loss, loss_l, loss_u, loss_ll, loss_uu, loss_ua, loss_la, loss_ub, loss_lb))
            # print('iteration %d : loss: %03f, loss_l: %03f, loss_u: %03f, loss_ll: %03f, loss_uu: %03f'%(iter_num, loss, loss_l, loss_u, loss_ll, loss_uu))
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./model/All/LA_{}_{}_labeled/pre_train".format(args.exp, args.labelnum)
    self_snapshot_path = "./model/All/LA_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
    ...
    shutil.copy('./code/LA_All_train.py', self_snapshot_path)
    # -- Pre-Training
